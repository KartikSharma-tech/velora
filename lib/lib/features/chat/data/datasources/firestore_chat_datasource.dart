...
class FirestoreChatDataSource {
  ...
  /// BUG FIX: read receipts were declared "done" but nothing ever
  /// flipped an individual message's `isSeen` to true — bubbles
  /// stayed on a single grey tick forever. Called whenever the
  /// recipient opens the thread; batch-updates every message sent
  /// *to* them that isn't marked seen yet.
  Future<void> markMessagesSeen({
    required String roomId,
    required String currentUserId,
  }) async {
    final unseen = await _chatRooms
        .doc(roomId)
        .collection('messages')
